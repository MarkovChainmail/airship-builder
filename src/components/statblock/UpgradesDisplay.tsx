...
export default function UpgradesDisplay({ upgrades, fuel, propulsion }: { upgrades: Upgrades, fuel: Fuel | undefined, propulsion: Propulsion | undefined }) {
    const {
        state,
        setter
    ...
    return (
        <div className="property-block shipside" onClick={onClick}>
            <h4>Upgrades</h4>
            <ul>
                <li key='protection'><p>Protection</p>
                    <ul>
                        <li key='armor'>{upgrades.armor} Hull Armor</li>
                        {
                            upgrades.kiteshield ? <li>Kite Shield</li> : null
                        }
                        {upgrades.magicshield == MagicShieldType.none ? null : <li key={'shield'}>{upgrades.magicshield} Shield</li>}
                    </ul>
                </li>
                <li key='propulsion'><p>Propulsion</p>
                    <ul>
                        {upgrades.sails ? <li key='sails'>{upgrades.sails} Sails</li> : null}
                        {propulsion ? <PropulsionList propulsion={propulsion}/> : null}
                    </ul>
                </li>
                {(upgrades.autopilot || upgrades.teleportationcircle) ?
                    <li key='misc'><p>Miscellaneous</p><ul>
                        {
                            upgrades.teleportationcircle ? <li key='tele'>Teleportation Circle</li> : null
                        }
                        {
